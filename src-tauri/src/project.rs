...
fn scan_project_files(project_dir: &PathBuf) -> (bool, u32, u32) {
    ...
    (has_outline, page_count, image_count)
}
        ...
        if entry.file_type().await.map(|t| t.is_dir()).unwrap_or(false) {
            ...
            if project_json.exists() {
                ...
                if let Ok(mut project) = serde_json::from_str::<ProjectInfo>(&content) {
                    ...
                    let (has_outline, page_count, image_count) = scan_project_files(&entry.path());
...
pub async fn open_project(
    ...
    // Update file status
    let (has_outline, page_count, image_count) = scan_project_files(&project_dir);
    project.has_outline = has_outline;
    project.page_count = page_count;
    project.image_count = image_count;
    project.has_images = image_count > 0;
